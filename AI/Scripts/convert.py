...
for txt_name in txt_name_list:
    ...
    for line in lines:
        ...
        if(len(line) >= 2):
            ...
            img_path = str('%s/images/%s.jpg'%(wd, os.path.splitext(txt_name)[0])) # path書き換えた2018.9.10
            ...
            txt_outfile.write(str(cls_id) + " " + " ".join([str(round(a,6)) for a in bb]) + '\n') # Edited 2018.9.10 桁数=2 round(n,2)

    """ Save those images with bb into list"""
    if(ct != 0):
        #list_file.write('%s/images/%s/%s.jpg\n'%(wd, cls, os.path.splitext(txt_name)[0]))
        list_file.write('%s/images/%s.jpg\n'%(wd, os.path.splitext(txt_name)[0])) # path書き換えた2018.9.10
