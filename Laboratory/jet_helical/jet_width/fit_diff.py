...
def gauss(y, q):
    result = (q[0]-q[1])/(1 + np.power(q[2]/(y[0]-q[3]), q[4])) + q[5]
    return result

"""
#r += (q[4]-q[5])/(1+(q[6]/(y[0]-q[7]))^q[8])
...
"""
# daikei
gau = ROOT.TF1("gauss", gauss, fit_min, fit_max, 6)
gau.SetParameters(paramater_0, paramater_1, paramater_2, paramater_3, paramater_4, paramater_5)
